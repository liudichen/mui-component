...
export const Cell = (props: any) => {
  ...
  const cellSx = useCreation(() => {
    const sx: SxProps = { px: 0, tableLayout: "fixed" };
  ...
  const value =
    rowIndex === -1 ? "" : valueGetter?.({ row, rowIndex, field, value: row?.[field] }) ?? row?.[field] ?? "";
  const cellProps = getCellProps?.({ row, rowIndex, field, value }) ?? {};
  const item =
    rowIndex === -1
      ? renderTitle?.({ field }) ?? title ?? field ?? ""
      : renderCell?.({ row, rowIndex, field, value }) ??
        (type === "status" ? (
          <StatusRender
            status={value}
          ...
          value ? (
            dayjs(value).format(type === "date" ? "YYYY-MM-DD" : "YYYY-MM-DD HH:mm:ss")
          ) : (
            "-"
          )
        ) : type === "boolean" ? (
  ...
  return (
    ...
    >
      {showTooltip &&
      (typeof showTooltip === "boolean" ||
        (typeof showTooltip === "function" && !!showTooltip({ value, row, rowIndex, field }))) ? (
